 ...
 */
class OptimizedBulletSpan(
        val bulletPointRadius: Int = STANDARD_BULLET_RADIUS,
    ...
    override fun getLeadingMargin(first: Boolean): Int {
        return 2 * bulletPointRadius + gapWidth
    }
    ...
    override fun drawLeadingMargin(
        ...
        if ((text as Spanned).getSpanStart(this) == start) {
            ...
            val yPosition = if (layout != null) {
                ...
                layout.getLineBaseline(line).toFloat() - bulletPointRadius * 2f
            } else {
                (top + bottom) / 2f
            }

            val xPosition = (x + dir * bulletPointRadius).toFloat()

            if (canvas.isHardwareAccelerated) {
                if (mBulletPath == null) {
                    mBulletPath = Path()
                    mBulletPath!!.addCircle(0.0f, 0.0f, bulletPointRadius.toFloat(), Direction.CW)
                }
                ...
                canvas.drawCircle(xPosition, yPosition, bulletPointRadius.toFloat(), paint)
            }
